...
@pytest.fixture
def panel():
    return dict(a=0.50, b=0.20)   # rib pitch × stringer pitch [m]
...
def test_Nxx_cr_cubic_thickness_scaling(mat, panel):
    # D ∝ h³ → Nxx_cr ∝ h³: doubling thickness → 8× critical load
    a, b = panel['a'], panel['b']
...
def test_Nxx_cr_narrower_panel_higher(mat, panel):
    # Narrower panel (smaller b) → higher Nxx_cr (b² in denominator)
    a = panel['a']
...
def test_bend_twist_warning_unbalanced(mat, panel):
    # A single off-axis ply (e.g. 30°) produces D16, D26 ≠ 0 → warning expected
    a, b = panel['a'], panel['b']
    lam = Laminate([Ply(mat, 0.125e-3, 30.0)])   # unbalanced: no -30° partner
    N = np.array([-1e3, 0.0, 0.0])
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        buckling_rf(N, lam.D, a, b)
    assert any("bend-twist coupling" in str(x.message) for x in w)


def test_no_bend_twist_warning_cross_ply(mat, panel):
    # Cross-ply [0/90/90/0] → D16 = D26 = 0 exactly → no warning
    a, b = panel['a'], panel['b']
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        buckling_rf(N, lam.D, a, b)
    assert not any("bend-twist coupling" in str(x.message) for x in w)
